...
for (N in N_values){
  ...
  for (t in 2:gen){
    ...
    analyt_stdv<-append(analyt_stdv,sqrt(v0[1]*(N-N*((N-1)/N)^t)))
    approx_stdv<-append(approx_stdv,sqrt(v0[1]*t))
  }
  ...
  if (N==N_values[3]){
  legend( "topleft",bty = "n", legend = c('emp. mean','emp. stdv','analyt. stdv','approx. stdv'), lty = c(4,1,2,3), col = 'black')}
